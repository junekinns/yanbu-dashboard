...
**지도**: 사우디 전역, 다크 베이스맵(CARTO), 레이어 5개를 토글 — 외교부 경보 도시(3단계 지명 + 주요 도시, 단계별 색), 최근 72시간 공격·요격·경보 사건(대상 도시 위치, 유형별 색, 매체 수 크기), 위성 이상 화점(7일, 전국), 항만 활동 링(8개 항, 평시 대비 색), 상시 플레어(기본 꺼짐). 점을 클릭하면 상세와 기사 링크.
